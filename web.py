...
USERS = {login: generate_password_hash(password) for login, password in users.items()}
...
@auth.login_required
def index():
    year_by_number: list[tuple[int, int]] = Game.get_year_by_number()
    last_year: int = year_by_number[0][0] if year_by_number else date.today().year

    games: list[Game] = Game.get_all_finished(sort=False)
    total_finished_game: int = len(
        [game for game in games if game.category == FINISHED_GAME]
    )
    total_finished_watched: int = len(
        [game for game in games if game.category == FINISHED_WATCHED]
    )
